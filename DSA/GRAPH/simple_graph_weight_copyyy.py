...
class Graph:
    ...
    def path_find(self, start, end):
        ...
        while len(s) > 0:
            ...
            p = self.d[k[-1][0]]
            for i in p.children:
                if self.check(i[0].info,k):
                    s.append(k + [(i[0].info,i[1]+k[-1][1])])
                    if i[0].info == end:
                        start_to_end_path.append(k + [(i[0].info,i[1]+k[-1][1])])
                    flag = True
            if flag is False:
                com_path.append(k)
...
while 1:
    ...
    if option == 1:
        ...
        h=G.path_find(s, e)
        print(G.shortest_path(h))
    elif option == 5:
        s, e = map(int, input('enter start and end').split())
        j=G.path_find_Dfs(s, e)
        print(G.shortest_path(j))
    else:
        break
